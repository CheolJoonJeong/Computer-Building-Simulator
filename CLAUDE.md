...
- **`CableSpawner`** (UI button) owns a `cableType`/`cablePrefab`/optional `defaultRoute`/`sourceSocket`; instantiates and initializes cables (`SpawnAt` → `cable.Init`), or detaches an assembled cable.
